  ...
  immer((set, get) => ({
      ...
      try {
        ...
        set((state) => {
          ...
          if (state.chapters[chapterIndex]?.bookId) {
            state.chapterCache[state.chapters[chapterIndex].bookId] =
              state.chapters;
          }
        });
    ...
    reorderChapters: async (bookId, chapterIds) => {
      ...
      set((state) => {
        ...
        if (state.chapterCache[bookId]) {
          state.chapterCache[bookId] = reorderedChapters;
        }
      });
      ...
      try {
        ...
        set((state) => {
          ...
          if (state.chapterCache[bookId]) {
            state.chapterCache[bookId] = previousChapters;
          }
    ...
    createScene: async (chapterId, bookId, data) => {
      try {
        const res = await bookAPI.scenes.create({
          chapterId,
          bookId,
          ...data,
        });
    ...
    reorderScenes: async (chapterId, sceneIds) => {
      ...
      set((state) => {
        const reorderedScenes = sceneIds
          ...
          .filter(Boolean);
        const otherScenes = state.scenes.filter(
      ...
      try {
        ...
        // Rollback
        set((state) => {
          state.scenes = previousScenes;
          if (bookId && state.sceneCache[bookId]) {
            state.sceneCache[bookId] = previousScenes;
          }
    ...
    moveSceneToChapter: async (sceneId, targetChapterId) => {
      ...
      try {
        const res = await bookAPI.scenes.moveToChapter(
          sceneId,
          targetChapterId,
        );
        if (!res.success) throw new Error(res.error);
